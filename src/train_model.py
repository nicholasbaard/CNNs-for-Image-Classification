...
def test(model, device, test_loader, epoch, criterion, writer):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
    ...
    writer.add_scalar('Loss/test', test_loss, epoch * len(test_loader))
    writer.add_scalar('Accuracy/test', accuracy, epoch * len(test_loader))
